...
INTEGER, PARAMETER :: nyears = 9   ! number of years in sim
...
INTEGER fileflag(23)               ! output file flag. CAUTION: if output file(s) added, need to re-dimension this array
...
INTEGER(kind=omp_nest_lock_kind), PRIVATE :: filelock(23)
CONTAINS
...
IMPLICIT NONE
INTEGER  SEEDS(8)
REAL(4), PARAMETER :: RM1=3.8580247E-6, RM2=7.4373773E-6 
...
!GRID(S)
INTEGER, PARAMETER :: cellsize = 2     ! size of one side of ea square cell (m)
...
INTEGER, PARAMETER :: topo = 3         !select DELFT 'topo' (i.e., elevation) input file (1=topo1(early), 2=topo2(middle), 3=topo3(late))
...
INTEGER watsched(9)            ! 9-year schedule of water years
...
INTEGER, PARAMETER :: inundationdays(inundationtimestamps) = (/ 241, 299, 300, 117 /)
INTEGER, PARAMETER :: inundationhours(inundationtimestamps) = (/ 5, 23, 4, 24 /)
...
INTEGER, PARAMETER :: speciesid(nfwsp) = (/ 1, 2, 3, 4 /)   ! ID numbers of food web sp (1=GS, 2=IS, 3=GK, 4=BA)
...
REAL(8), PARAMETER :: ageclasspopsize(nfwsp) = (/ ageclassspace(1)*30.D0, & ! sp-specific pop size in ea age class ("real" indivs, not super-indivs)
                                                  ageclassspace(2)*15.D0, &
                                                  ageclassspace(3)*20.D0, &
                                                  ageclassspace(4)*50.D0 /)
...
INTEGER, PARAMETER :: cellmax = 700    ! total model indivs allowed in ea cell
...
REAL(8) bmassbycell(nfwsp,cols,rows)   ! total of biomass (g wwt) of each sp in ea cell
...
INTEGER, PARAMETER :: minfeedhr = 7  !earliest hour in day that indiv of all sp can feed (defines feeding window)
INTEGER, PARAMETER :: maxfeedhr = 18 !latest hour in day that indiv of all sp can feed (defines feeding window)
INTEGER, PARAMETER :: totfeedhr = maxfeedhr - minfeedhr + 1 !number of hours in feeding window
...
IMPLICIT NONE
INTEGER i, j, ifwsp
INTEGER num_threads, omp_chunk

num_threads = 4!omp_get_num_procs()
...
fileflag(5)  = 0 !do (1) or do not (0) output 'out_gridhabitat.m' (file 100, habitat type in ea cell)
fileflag(6)  = 0 !do (1) or do not (0) output 'out_gridinundhrs.m' (file 77, total hours inundated in a year for ea cell)
...
OPEN (81, file = 'waterlevels1_0708.dat', STATUS = 'UNKNOWN')!input raw and 5-d averaged hourly water levels from Atchafalaya Delta, 3/1/2007-2/23/08 (Station ID 8764227)(m)
OPEN (82, file = 'waterlevels2_0809.dat', STATUS = 'UNKNOWN')!input raw and 5-d averaged hourly water levels from Atchafalaya Delta, 3/1/2008-2/23/09 (Station ID 8764227)(m)
OPEN (83, file = 'waterlevels3_0910.dat', STATUS = 'UNKNOWN')!input raw and 5-d averaged hourly water levels from Atchafalaya Delta, 3/1/2009-2/23/10 (Station ID 8764227)(m)
OPEN (84, file = 'waterlevels4_1011.dat', STATUS = 'UNKNOWN')!input raw and 5-d averaged hourly water levels from Atchafalaya Delta, 3/1/2010-2/23/11 (Station ID 8764227)(m)
OPEN (85, file = 'waterlevels5_0607.dat', STATUS = 'UNKNOWN')!input raw and 5-d averaged hourly water levels from Atchafalaya Delta, 3/1/2006-2/23/07 (Station ID 8764227)(m)

!--open output files---------------------------------------------------
OPEN (12, file = 'out_input_echo.out', STATUS = 'UNKNOWN')      !output all input and parameter values
OPEN (22, file = 'out_indivs_alive.out', STATUS = 'UNKNOWN')    !output traits of alive indivs
OPEN (33, file = 'out_allocatedspace.out', STATUS = 'UNKNOWN')  !output space allocated or wasted
OPEN (44, file = 'out_stage_abunds.out', STATUS = 'UNKNOWN')    !output abundance in ea stage and sp
OPEN (55, file = 'out_gridelevation.m', STATUS = 'UNKNOWN')     !output elevation (m) of ea cell
OPEN (66, file = 'out_griddepth.m', STATUS = 'UNKNOWN')         !output water depth (m) of ea cell
OPEN (77, file = 'out_gridinundhrs.m', STATUS = 'UNKNOWN')      !output total hours inundated in a year for ea cell
OPEN (99, file = 'out_gridfishbycell.out', STATUS = 'UNKNOWN')  !output sp-specific totals for superindiv, worth (abundance), and biomass in ea cell
OPEN (100, file = 'out_gridhabitat.m', STATUS = 'UNKNOWN')      !output habitat type in ea cell
OPEN (110, file = 'out_indivs_added.out', STATUS = 'UNKNOWN')   !output initial attributes of all indivs that are allocated (recruited) or initialized
OPEN (120, file = 'out_indivs_dead.out', STATUS = 'UNKNOWN')    !output traits of dead indivs (nat mort, starved, stranded, predated, old)
OPEN (130, file = 'out_indivs_move.out', STATUS = 'UNKNOWN')    !output all movement info (including sub-hour) for up to 5 user-defined indivs
OPEN (140, file = 'out_stage_means.out', STATUS = 'UNKNOWN')    !output mean length, weight, and density of ea stage and sp
OPEN (150, file = 'out_indivs_diet.out', STATUS = 'UNKNOWN')    !output indiv diets: g wwt consumed by prey type, species, stage
OPEN (160, file = 'out_gridpreybycell.out', STATUS = 'UNKNOWN') !output abundance and biomass of 5 base prey types by unit (m**2 or m**3) and cell
OPEN (170, file = 'out_gridpreymeans.out', STATUS = 'UNKNOWN')  !output mean abundance and biomass (#/m**2 or m**3 and cell) of 5 base prey types by hab type and grid
OPEN (180, file = 'out_lifetable.out', STATUS = 'UNKNOWN')      !output life table by sp and stage
OPEN (190, file = 'out_summary_diet.out', STATUS = 'UNKNOWN')   !output mean diet by sp and prey type
OPEN (200, file = 'out_summary_habdist.out', STATUS = 'UNKNOWN')!output mean abund-based habitat distribn by sp and hab type
OPEN (210, file = 'out_summary_maxden.out', STATUS = 'UNKNOWN') !output highest density (worth/m**2) on grid by sp
OPEN (220, file = 'out_summary_worthlost.out', STATUS = 'UNKNOWN')!output worth lost by sp to nat mort, starvation, stranding, predation
OPEN (230, file = 'out_summary_profiling.out', STATUS = 'UNKNOWN')!output worth lost by sp to nat mort, starvation, stranding, predation
OPEN (240, file = 'out_gridproductivity.out', STATUS = 'UNKNOWN')!output net productivity (g wwt = weight lost or gained or lost * worth) by species and cell)
...
WRITE (240, 2401)
2401 FORMAT ('year', 1X, 'fw_sp', 1X, 'col', 1X, 'row', 1X, 'net_prod')
...
DO jday = 1, ndays
 ...
 mortloss=0.D0
 notcalculatedspawningdays=0                                                                   !zero daily, sp-specific record of worth lost to nat mort, starvation, stranding, predation
...
DO hour = 1, nhours
 ...
 indivbycell=0; worthbycell=0.D0; bmassbycell=0.D0; mxwrth=0.D0; maxcoord=0; habdist=0.D0; idbycell=0 !zero fish-by-cell sums and max worth obs in hr, corresponding coord, and abund by sp and hab type (from 'fishbycell')
...
!  NEEDS TO BE RE-CALIBRATED  IF (hour .EQ. maxfeedhr) CALL forcecons !if last hr of feeding window, fix daily cons at prop'n of max. Use to check/calibrate bioen. Affects movement b/c base prey not actually consumed
...
END DO  !julian day loop dddddddddddddddddddddddddddddddddddddddddddddd end days
 ...
 IF (watyr .GT. 9) watyr = 1        !if 10+,  reset to first water year (only 9 water years)
...
USE physicalenv
IMPLICIT NONE
REAL(8) inputelev(delftcols,delftrows) !input elevation (interpolated DELFT data; m)
INTEGER ii, jj

DO jj = 1, delftrows                               !loop over all rows in DELFT grid
 READ (11, 111) (inputelev(ii,jj),ii=1, delftcols) !read in elevation data (m)
...
DO ii = 1, cols                              !loop over sub-grid
 DO jj = 1, rows
  cellelev(ii,jj) = inputelev(subgrid(1)+ii-1,subgrid(3)+jj-1) !extract sub-grid elevations (m)
 END DO !columns
END DO !rows
...
INTEGER inundstore(5,cols,rows)!stored inund values (one for each water level profiles used in model)
REAL(8) watdep(cols,rows)      !water depth (m) in ea cell (local to avoid conflict w/ global array 'waterdepth')
INTEGER xx, yy
INTEGER ii, jj
...
IF (inundflag(wlp) .EQ. 1) THEN           !if inundation already calculated for given water level profile
...
IF ((emersearch .EQ. 0 .AND. nhdflag .EQ. 1) .OR. emersearch .EQ. 1) THEN !if eval current cell and in new environment OR eval for 'emernhood'
 DO xx = 1, 3                    !look 3 hours into future
  ...
  IF (futrhr .GT. ndays*nhours) THEN !if future hr > max hrs in year
   ...
   IF (watyr .NE. 9) futrdep = waterlevel(watsched(watyr+1),futrhr) - cellelev(c,r) !if not last water yr, use 1st hr of next yr to calc future depth (water level - cell elev) (m)
   IF (watyr .EQ. 9) futrdep = waterlevel(1,futrhr) - cellelev(c,r) !if last water yr, use 1st hr of 1st water yr, to calc future depth (water level - cell elev) (m)
 ...
 DO ii = -nhd, nhd          !loop over all columns in neighborhood
  ...
  DO jj = -nhd, nhd         !loop over all rows in neighborhood
   ...
   IF (cumhr-(year-1)*ndays*nhours+1 .GT. ndays*nhours) THEN !if future hr > max hrs in yr
    IF (watyr .NE. 9) futrdep = waterlevel(watsched(watyr+1),futrhr) - cellelev(c+ii,r+jj) !if not last water yr, use 1st hr of next yr to calc future depth (water level - cell elev) (m)
    IF (watyr .EQ. 9) futrdep = waterlevel(1,futrhr) - cellelev(c+ii,r+jj)         !if last water yr, use 1st hr of 1st water yr, to calc future depth (water level - cell elev) (m)
...
REAL(8) worthchng !change in worth as result of nat mort
...
worthchng = xworth(ind) - xworth(ind) * EXP(-xmort(ind)* mortmult(ifwsp,habitat(c,r))) !calc change in worth as result of nat mort (mod by predation via sp-specific hab multiplier)
xworth(ind) = xworth(ind) * EXP(-xmort(ind)* mortmult(ifwsp,habitat(c,r))) !use mortality to decrement worth
...
IF (waterdepth(c,r) .GE. xmindep(ind)) THEN           !if indiv in deep enough water after 'j' timesteps
 ...
 mortloss(ifwsp,1) = mortloss(ifwsp,1) + worthchng    !add change in worth to daily running tot of worth lost to nat mort
 cummortloss(1) = cummortloss(1) + worthchng          !add change in worth to annual running tot of worth lost to nat mort
 IF (xsensce(ind) .EQ. 0) netproductivity(ifwsp,c,r) = netproductivity(ifwsp,c,r) - (worthchng * xtotwght(ind)) !if individula is not senescent, subtract change in biomass from annual running total (g wwt)
 IF (xworth(ind) .LT. minworth) THEN                  !if updated worth very small
  ...
  IF (xsensce(ind) .EQ. 0) netproductivity(ifwsp,c,r) = netproductivity(ifwsp,c,r) - (xworth(ind) * xtotwght(ind)) !if individula is not senescent, subtract remaining biomass from annual running total (g wwt)
...
ELSE                                                  !otherwise, water too shallow for indiv (indiv can't move)
 IF (fileflag(13) .EQ. 1) THEN                        !if output desired
  outflag = 13; CALL output(0,0,0,0,0,0,0)            !output details of fine-scale movement (13=file 130)
 END IF
 mortloss(ifwsp,3) = mortloss(ifwsp,3) + worthchng    !add change in worth to daily running tot of worth lost to stranding
 cummortloss(3) = cummortloss(3) + worthchng          !add change in worth to annual running tot of worth lost to stranding
 IF (xsensce(ind) .EQ. 0) netproductivity(ifwsp,c,r) = netproductivity(ifwsp,c,r) - (worthchng * xtotwght(ind)) !if individula is not senescent, subract change in biomass from annual running total (g wwt)
 ...
 IF (xworth(ind) .LT. minworth .OR. hoursdry(ind) .GT. 5 & !if updated worth very small OR >5 hours in dry cell
  ...
  ELSE                                                !otherwise, updated worth very small OR <100 sp-specific strandings OR indiv not within 5 cells of sub-grid edge
   ...
   IF (xsensce(ind) .EQ. 0) netproductivity(ifwsp,c,r) = netproductivity(ifwsp,c,r) - (xworth(ind) * xtotwght(ind)) !if individula is not senescent, subtract stranded biomass from annual running total (g wwt)
...
IF (xalive(ind) .EQ. 2 .OR. xalive(ind) .EQ. 4) THEN  !if indiv dead b/c of nat mort (incl. predation; 2) or stranding (4)
 ...
 xworth(ind)=0.D0; xsomchng(ind)=0.D0; xgonchng(ind)=0.D0; xtotchng(ind)=0.D0; xlenchng(ind)=0.D0 !zero worth !som, gon, and total wght changes (g wwt)!length change (mm)
ENDIF
END SUBROUTINE
...
xtotchng(indiv) = xsomchng(indiv) + xgonchng(indiv)     !update total wght change (g wwt)
IF (xsensce(indiv) .EQ. 0 .AND. xsomwght(indiv) .GT. xstrvwght(indiv)) & !if indiv not senescent AND not going to starve, add total weight change to annual running total (g wwt)
 netproductivity(ifwsp,c,r) = netproductivity(ifwsp,c,r) + (xtotchng(indiv) * xworth(indiv))
IF (xsensce(indiv) .EQ. 0 .AND. xsomwght(indiv) .LT. xstrvwght(indiv)) & !if indiv not senescent AND going to starve, subtract INITIAL total weight from annual running total (g wwt)
 netproductivity(ifwsp,c,r) = netproductivity(ifwsp,c,r) - ((xtotwght(indiv)+xtotchng(indiv)) * xworth(indiv))
...
IF (xsomwght(indiv) .LT. expsomwght) THEN               !if new som wght < expected wght (g wwt)
 IF (xsomwght(indiv) .LT. xstrvwght(indiv)) THEN        !if new som wght < starvation wght (g wwt)
  ...
  IF (fileflag(11) .EQ. 1 .AND. (fwsp .EQ. isspeciesout .OR. isspeciesout .EQ. 0)) THEN !if output desired AND (want specific species OR all sp)
   outflag = 12; CALL output(0,0,0,0,0,0,0)             !output attributes of dead indiv (12=file 120)
  END IF !output desired (dead indivs)
  xworth(indiv)=0.D0; xsomwght(indiv)=0.D0; xgonwght(indiv)=0.D0; xtotwght(indiv)=0.D0; xlenchng(indiv)=0.D0 !zero worth, set som, gon, and total wghts to zero (g wwt), record a zero change in length (mm)
...
DO xx = 1, nfwsp                             !loop over food web sp, print error if sum of worth across stages < minimum
 IF (SUM(ageabund(xx,:)) .LT. minworth) PRINT *, "Food web species", xx, "extinct at end of year", year
END DO !species
...
IF (assignflag .EQ. 3) THEN          !if unused space at end of year, split high-worth indivs (called from 'recruitment')
 ...
 IF (maxind .NE. 0) THEN             !if at least one indiv of sp xx alive to split
  ...
  xsomchng(ind)=0.D0; xgonchng(ind)=0.D0; xtotchng(ind)=0.D0 !these will be defined by bioenergetics (g wwt)
  xlenchng(ind) = 0.D0               !mm
...
IF (outflag .EQ. 23) THEN !output daily worth lost to nat mort, starvation, stranding, predation, senescence (file 220) !22_22_22
 !IF (MOD(year,iswyearout) .EQ. 0 .AND. MOD(jday,iswdayout) .EQ. 0) THEN !and limit to MOD = 0
   ...
   2302 FORMAT (1X, I3, 1X, I3, 3X, I2, 5X, I2, 5X, ES10.4)
 !END IF !output limits
END IF !flag 22

    IF (outflag .EQ. 24) THEN !output cummulative annaul net change in productivity by species and cell (file 230) !23_23_23
        IF (MOD(year,iswyearout) .EQ. 0) THEN !and limit to MOD = 0
            DO c = 1, cols !loop over columns
                DO r = 1, rows !loop over rows
                    WRITE (240, 2402) year, i, c, r, netproductivity(i,c,r)
2402                FORMAT (1X, I3, 1X, I1, 1X, I3, 1X, I3, 1X, ES10.3)
                END DO !rows
            END DO !columns
        END IF !output limits
    END IF !flag 23
CALL file_unlock(outflag)
